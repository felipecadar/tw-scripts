...
def getLastFiles(root:str="./tw", world:str="br114"):
...
def filterVillages(villages:List[Village], K:int = -1, min_points:int=-1, max_points:int=-1, remove_players:List[int]=[], only_players:List[int]=[], types:List[Vtype] = [], distance_to_center:int = -1, center:np.ndarray = np.array([])) -> List[Village]:
    if K >= 0:
